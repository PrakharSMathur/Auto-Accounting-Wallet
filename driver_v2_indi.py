...
while True:
    ...
    if s.in_waiting:
        ...
        if text=="S":
            ...
            for i in l_names:
                camera.capture(i)
            #buf=s.readline().decode('utf8').strip()
            print("Waiting")
            f=0
            #if not buf:
            while not s.in_waiting or not dcn:
            ...
            sno_last=sno_l[-1][0]
            '''
            c.execute("insert into testtable(SNo,In,Out,Location,Timestamp) values (%s,%s,%s,%s,%s);",(sno_last+1,in_val,out_val,null,default))
            db.commit()
            '''
            ...
            if dcn=="In":
                ...
                g = geocoder.ip('me')
                print(g.latlng)
                c.execute("insert into testtable values (%s,%s,%s,%s,%s)",(sno_last+1,note_val,0,str(g.latlng),strftime('%Y-%m-%d %H:%M:%S')))
                db.commit()
                ...
                print(g.latlng)
                #Query for Out
                c.execute("insert into testtable values (%s,%s,%s,%s,%s)",(sno_last+1,0,note_val,str(g.latlng),strftime('%Y-%m-%d %H:%M:%S')))
                db.commit()
